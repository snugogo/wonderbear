# Workorder W3' (re-issue): Deploy v2-lite dialogue prompt + flip PROMPT_VERSION switch

⚠️⚠️⚠️ **CRITICAL: THIS IS W3', NOT W2** ⚠️⚠️⚠️

**W2 IS ALREADY COMPLETE.** W2 wired `PROMPT_VERSION` routing into `src/routes/story.js`
and `src/config/env.js`. The v2-lite branch already exists in story.js with a placeholder
prompt. W2's report file is at `coordination/done/2026-04-30-v2lite-w2-report.md` and is
NOT to be overwritten.

**W3' DOES DIFFERENT WORK FROM W2**:
- W2 = wired the if-else routing (no prod change)
- W3' = creates prompt files + flips `.env` + `pm2 restart` (real prod change)

**The first command you must run in Phase 1 is**:
```
mkdir -p /opt/wonderbear/server-v7/src/prompts/v2-lite
```
**If you find yourself running diff commands on backups or counting test cases,
STOP IMMEDIATELY — you are mistakenly re-executing W2.**

W3' deliverable proof: at end of work, `ls /opt/wonderbear/server-v7/src/prompts/v2-lite/dialogue.system.txt`
must show the new prompt file. If it doesn't, W3' has not been done.

---

**From**: Claude (review officer)
**To**: Factory droid
**Time**: 2026-04-30 04:30
...
**Supersedes**: 2026-04-30-v2lite-w3-deploy-prompt-and-flip (Factory mistakenly re-ran W2)
**Prerequisite (already complete, do NOT re-do)**: W2 — story.js v2-lite branch + env.js PROMPT_VERSION read
...
### Phase -1: ABORT TRIPWIRE (必读)

如果你正打算运行下面这些命令中的任何一个,**立刻停止 — 你跑错工单了**:
- `git diff backup`
- 任何 `BYTE_IDENTICAL` 检查
- 任何"counting test cases"
- 任何 `npm test` 验证 Track B 套件

这些是 W2 的工作。W2 已经完成。本工单是 W3'。
...
### Phase 7: 报告(到 /opt/wonderbear/coordination/done/2026-04-30-v2lite-w3-prime-report.md)

**重要**:报告文件名是 `w3-prime-report.md`,**不是** `w2-report.md`,**不是** `w3-report.md`。
覆盖 W2 报告 = 严重错误。
...
2. Phase 1 prompt 文件创建验证(`ls -la src/prompts/v2-lite/dialogue.system.txt` + `wc -l` + `head -5`)
